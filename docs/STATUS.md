...
| 사람 판단이 필요한 것 | RAG 평가셋 확장분 150건(51~200번) 판정 — Claude 1차 판정에 ANTHROPIC 키 재투입 필요, 그다음 사람 검수(§4-4) |
...
- **평가셋 확장(9/24 저녁, v0.37.0)** — gemma3로 funding 110 + news 40 추가 → **200건**(candidate 150, 시트 51~200번).
  뉴스 40건은 처음 들어간 뉴스 표본이라 corpus 성격(인사·분양·팝업 등 상권 무관 기사 다수)이 그대로 드러난다.
  **대기: 150건 Claude 1차 판정** — `.env`에 `ANTHROPIC_API_KEY`가 없어(22:43 제거됨) 못 돌렸다. 키를 넣거나
  `ANTHROPIC_API_KEY=… python -m apps.rag.adapter.inbound.cli.judge_evalset`로 한 번 실행 → 사람 검수 → apply → evaluate.
