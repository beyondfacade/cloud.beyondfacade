...
| 사람 판단이 필요한 것 | 평가셋 200건 판정 완료(confirmed 171, fp16 R@5 0.97·MRR 0.90). 결정 대기 2: 뉴스 다중 정답 전환 · 수치 과다 질문 완화(§4-4) |
...
- **평가셋 확장(9/24 저녁, v0.37.0)** — funding 110 + news 40 추가 → **200건**(candidate 150, 시트 51~200번).
  ...
  **O 141 / X 9, X 집합 동일**(155 인천 누락 + 뉴스 동일 사건 중복 8건). 그대로 반영 → **confirmed 171 / rejected 29**.
  ...
  두 판정이 남긴 숙제(결정 대기): ① 뉴스는 같은 보도자료 기사가 여럿이라 단일 정답 구조와 충돌 — 사건 단위
  다중 정답(multi-positive)으로 바꿀지. ② Fable 지적: 51~200 질문에 금리·한도 등 수치가 많아 어휘 일치로 풀리는
  "정답 누출" — 수치 2개 이상·공고 내 고유명사 질문 10여 건(55·85·87·88·95·114·126·130·136·149)을 덜어낼지.
  ③ GPT 지적: 뉴스 `matched_keyword`(수집 키워드)가 기사 핵심 지역과 어긋나는 예 9건(170 안양→관악구 등) —
  이건 수집 키워드일 뿐 NER 태그가 아니라 평가엔 무관하지만, 뉴스를 지역 신호로 쓸 때는 본문 지역 추출이 따로 필요.
