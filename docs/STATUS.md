...
| 사람 판단이 필요한 것 | 평가셋 2차 완화 6건 재판정(`재판정_요청_2차_6건.md`) — 그 외 완료(confirmed 174, fp16 Hit@5 1.00·MRR 0.95, §4-4) |
...
- **평가셋 확장(9/24 저녁, v0.37.0)** — funding 110 + news 40 추가 → **200건**(candidate 150, 시트 51~200번).
  ...
  **9/24 밤 결정 2건 실행(v0.37.1)**: ① 뉴스 다중 정답 — 40건 중 29건에 같은 사건 기사를 relevant_ids로 묶음
  ...
  같은 기준으로 수치 2개 이상 남은 9건 중 6건(78·104·105·120·137·138)을 2차 완화 → **candidate 6, 재판정 대기**
  (`data/eval/재판정_요청_2차_6건.md`). 131·132·141은 같은 시 유사 공고와 수치로만 갈려 유지.
  현재 **confirmed 174 / rejected 20 / candidate 6**.
  남은 관찰: GPT 지적한 뉴스 `matched_keyword`는 수집 키워드일 뿐 NER 태그가 아니라 평가엔 무관 — 뉴스를 지역 신호로
  쓸 때 본문 지역 추출이 따로 필요. 뉴스 corpus는 보도자료 복제가 대부분이라 색인 전 사건 단위 dedupe가 다음 개선 후보.
