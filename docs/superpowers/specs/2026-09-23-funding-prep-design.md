...
- **지역 — `hashtags`로 판정한다**(쉼표 구분 태그, 앞쪽에 분야·시도 태그가 온다). 2026-09-24 실측
  (미만료 2,032건 → 서울 전용 70 · 전국 490 · 제외 1,472):
  · `서울`이 있고 전 시도를 덮지는 않음 → **서울 전용**
  ...
  · **원천이 시도 어휘를 두 벌 섞어 쓴다** — `전남광주`(통합 명칭) 512건과 `광주`·`전남`(282·281건)이
    ...
    "서울 전용"으로 오분류된다. `전남광주`를 펴서 17개 기준 커버리지로 판정한다(T4-1에서 발견·정정).
    `전국` 태그는 3건뿐이라 그것만 믿을 수 없다
...
- 정렬: 서울 전용 → 전국 순, 그 안에서 마감 임박 순(상시는 뒤), 상위 **8건**. 실측 풀은 92건(소상공인·창업 ∩ 서울/전국 − 서울 밖 지자체 = 서울 전용 18 + 전국 74). 각 항목에 `why`(어느 규칙에 걸렸는지 한 줄 — "서울·소상공인·금융")
