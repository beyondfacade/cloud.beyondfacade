...
지표 하나당 프론트가 아는 것은 **여섯**이다: 목록(`METRICS`) · 라벨(`METRIC_LABELS`) ·
색 스킴(`METRIC_SOURCES.scheme`) · 표기 형식(`FORMAT_BY_METRIC`) · 원천(`METRIC_SOURCES`) ·
**데이터 보유 범위(`metric-coverage.ts`)**.

여섯 번째만 `METRIC_SOURCES`의 항목 필드가 아니다. 앞의 다섯은 지표만 보면 정해지는데 보유 범위는
**(업종, 지표) 쌍**에 걸리기 때문이다 — 스냅샷 원천 업종(어린이집·편의점)은 개폐업 이력이 없어
관측 연도의 점포수 한 칸만 갖는다. 지표당 한 줄로 표현되지 않아 별도 모듈을 뒀다(v0.24.0).

2026-09-24 실측 근거:

| 축 | 조합 | 범위 |
|---|---|---|
| region_quarter | 동네 지표 4종 | 20211~20262 전 22분기 × 422행 — **공백 없음** |
...
mock 계약 테스트(`api/mock/metrics/route.test.ts`)가 실 API 실측값과 대조해 한 겹 더 막는다 —
...
**재평가 (2026-09-24, v0.24.0 작업 중).** 지표당 아는 것이 다섯에서 여섯으로 늘었지만 전환 조건은
**아직 둘 다 아니다** — 지도 지표는 여전히 7개(동네 4 + 업종 3)이고, 백엔드 지표가 화면에 안
나타나는 드리프트는 실제로 일어나지 않았다. 다만 여섯 번째가 `(업종, 지표)` 쌍이라 카탈로그
스키마로 옮길 때 `map_metric` 한 테이블로는 부족하다는 것이 드러났다 — 전환한다면 보유 범위는
별도 행 집합(`map_metric_coverage`)이 되거나, 백엔드가 계산해 내려주는 값이어야 한다.
