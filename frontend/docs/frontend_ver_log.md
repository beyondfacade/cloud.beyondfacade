...
### Added
- **AI 분석 폼 예시 질문 칩** — 업종별 3개(`features/agent-report/lib/example-questions.ts`), 누르면 추가 질문
  입력칸을 그 문구로 채우고 첫 문구가 placeholder. 업종을 바꾸면 칩이 바뀌지만 이미 적은 질문은 지우지 않는다.
  대구 분화본 구조 이식이되 문구는 리포트 5개 섹션(판정·상권·충격·자금·계산)이 답할 수 있는 축으로 다시 썼다 —
  여기 에이전트는 finance 도구가 있어 손익분기·월세 계산 질문도 예시에 넣었다(대구는 제외했었음).
  미등록 업종은 일반 문구 1개로 폴백. 칩 스타일은 `analysis-workspace.module.css`에 둠(feature 간 import 금지)
- 테스트 7건: lib 4(10업종×3·서울 맥락·40자 이내·폴백) + 폼 3(칩 표시·클릭→제출값·업종 전환 시 입력 유지)

## [v0.24.1] - 2026-09-24

### Changed
- **학원도 "개폐업 이력 없는 업종"으로** — 백엔드 v0.35.3부터 학원 폐업률·성장률이 NULL이라 실 API가 빈 배열을
  준다. `map-state.ts`에 `NO_CLOSURE_HISTORY_INDUSTRIES`(스냅샷 2종 + academy)를 두고
  `isMetricMissingForIndustry`·지도 배너·사이드패널 안내가 이 집합을 본다. 스냅샷 집합(`SNAPSHOT_INDUSTRIES`)은
  "점포수가 관측 연도 하나뿐"이라는 별개 규칙이라 그대로 — 학원 점포수는 전 연도에 있다
- 안내 문구를 원천 중립으로: "이 업종의 원천에는 개폐업 이력이 없어 폐업률·성장률이 없습니다"
- mock 계약 테스트·커버리지 테스트에 학원 케이스 추가 (실 API 미러, §15)

## [v0.24.0] - 2026-09-24

### Added
- **지표별 데이터 보유 범위를 단일 원천으로** (`map-explorer/lib/metric-coverage.ts`) — 셀렉터가
  "고를 수는 있지만 빈 지도가 되는" 조합을 제안하지 않는다. 컨트롤바의 연도 옵션이 현재 (업종, 지표)의
  보유 범위만 그리고, 범위 밖 시점은 조용히 가장 가까운 유효 시점으로 당긴다(`clampToCoverage`, 멱등).
  URL로 직접 들어온 조합도 `map-page`가 보정하고 주소를 맞춘다

### Fixed
- **mock이 실 API의 빈 응답을 흉내 내지 않던 것 (§15 미러 규칙 위반)** — `/api/mock/metrics`가 어떤
  조합에도 427행을 주고 있었다. 실 API는 스냅샷 업종(어린이집·편의점)에 **관측 연도의 점포수만**
  준다. mock으로 개발하는 동안 빈 지도 경로를 한 번도 못 보게 되어 있었다. 픽스처가 같은 커버리지
  모듈을 쓰게 해 실 API와 같은 공백을 재현한다
- `quarters.ts`의 낡은 주석 — "commerce 계열은 20254까지라 마지막 두 분기가 빈 지도"는 **사실이
  아니다**. 실측 결과 동네 지표 4종(`operating_months` 포함) 모두 20211~20262 전 22분기가 422행씩
  차 있다. 20254까지인 것은 매출·점포·시간대 어긋남 테이블이고 그쪽은 단계구분도가 아니라
  사이드패널 차트가 쓰며 이미 404 + 안내로 처리된다

### 실측 근거 (2026-09-24, 실DB + 실 API)

| 축 | 조합 | 범위 |
|---|---|---|
| region_quarter | `neighborhood_type`·`night_index`·`fnb_share`·`operating_months` | 20211~20262 전 22분기 × 422행 |
| industry_year | 일반 8업종 × 3지표 | 2019~2026 전 연도 (415~427행) |
| industry_year | `childcare`·`convenience_store` × `store_count` | **2026년만** (426·427행) |
| industry_year | `childcare`·`convenience_store` × `closure_rate`·`growth_rate` | **전 연도 0행** (원천에 개폐업 이력 없음) |

### 설계 판단
백엔드 커버리지 엔드포인트(카탈로그의 축소판)를 만들지 않았다. `map-metric-contract.md` §6의 전환
조건(드리프트 실발생 · 지표 8개 초과) 어느 것도 아직 아니다. 또 실제 공백은 **지표별 분기 범위가
아니라 (업종, 지표) 쌍**에 걸려 `METRIC_SOURCES`의 항목 필드로는 표현되지 않는다 — 그래서 별도 모듈이다.

**연도를 좁히지 않는 경우를 일부러 남겼다.** 스냅샷 업종 × 폐업률은 어느 연도에도 없는데, 연도를
걸러내면 원인(지표)이 가려진다. 기존 안내("스냅샷 원천이라 이 지표는 아직 없습니다. 점포수를 선택해
보세요.")가 설명하게 둔다.

### Validation
- 테스트 17건 추가(커버리지 규칙 8 · 셀렉터 4 · mock 계약 2 · 보정 3) → vitest **280/280**(64파일), `tsc` clean
- 실 dev 서버: `?industry=childcare&metric=store_count&year=2020` → `year=2026`으로 보정·연도 옵션 1개 ·
  `?metric=operating_months&year_quarter=20262` → 빈 지도 아님(범례 8구간) ·
  스냅샷 × 폐업률 → 기존 안내가 원인을 설명

## [v0.23.0] - 2026-09-24

### Added
- **`/plan` ⑤ 조달·상담 준비** — 계산·비교로 끝나던 흐름에 "무엇을 물어볼지"를 붙였다. 결과물은
  상품 목록이 아니라 **조정한 계획 + 후보 + 확인할 질문**이다(설계서 §5). 새 feature를 만들지 않고
  `features/plan/`에 이어 붙였다 — 같은 계획 흐름이다
  - `profile-form.tsx` — 창업 단계 5항. **`null`(아직 묻지 않음)과 `"unknown"`(모른다고 답함)을
    구분해 보관한다.** 라디오에 기본 선택을 두지 않아 "안 물어본 것"이 "아니오"로 굳지 않는다.
    모른다고 둔 항목은 질문 목록에 확인할 것으로 남는다
  - `candidate-cards.tsx` — 후보 8장. **"자격 확정이 아니라 해당 가능성"** 고지를 목록 위에 고정하고
    원문 링크를 `target="_blank" rel="noreferrer"`로 연다. 조달 필요 금액을 기준으로 찾았다고 밝힌다
  - `question-list.tsx` — 서버가 주는 건 초안이다. 인라인 편집·삭제·순서 변경·직접 추가.
    **편집본이 정본**이라 다시 조회해도 덮지 않는다(`useRef` 한 번만 받기)
  - `prep-sheet.tsx` + `lib/prep-markdown.ts` — 한 화면에 계획·비교·변경 이유·후보·질문·프리필 단서.
    Markdown 복사 / 질문만 복사 / 인쇄. **숫자는 서버 결과를 그대로 쓰고 여기서 다시 계산하지 않는다**
  - `plan-draft` v2 — `profile`·`questions`·`candidates_seen` 추가. 동네·업종이 바뀌면 후보·질문은
    버리고 **사람이 쓴 것(변경 이유·창업 단계)은 남긴다.** v1 초안은 되살리지 않는다(구조가 달라
    화면이 `undefined`를 만진다)
  - mock `/api/mock/funding/candidates`·`/api/mock/finance/questions` — 실 API 미러. 질문 mock은
    요청에 **계산 결과를 받지 않고** `input`으로 다시 계산한다(실 API와 같은 계약)

### 문구 결정
- **은행에 보내는 것처럼 보이는 버튼을 두지 않는다.** 전송·제출·신청·예약 버튼이 없음을 테스트로
  고정했다. 저장은 복사·인쇄로 사용자 몫이다
- **금지 문구 스캔**("충분합니다"·"신청 완료"·"승인되었")을 `features/plan` 전체 소스에 건다.
  처음엔 `result-figures.tsx`를 잡았는데 그건 **규칙을 적어둔 주석**이었다 — 주석을 걷어낸 뒤
  보도록 고치고, 그 예외가 의도된 것임을 별도 테스트로 남겼다

### Validation
- 테스트 31건 추가 → vitest **266/266**(63파일), `tsc --noEmit` clean
- 실 dev 서버(3200→8201): `/plan` 200, 후보 8건("서울 · 창업 · 금융"), 질문 9개
  (조달 2 · 가정 4 · 절차 3 — 조달 필요 3,151만·부족액 651만)
- E2E `[10/10]` 추가 — 관문부터 준비자료까지 **전 구간 통과**: 리포트 4,207자 · 조달 필요 348만 원 ·
  후보 8건 · 질문 8개 · 준비자료 2,900자 · Markdown 복사 버튼

## [v0.22.1] - 2026-09-24

### Fixed
- **E2E `[9/9]` 제출 버튼이 화면 밖** — 13필드 폼이라 "계산하기"가 실측 `top 1356px` / 뷰포트 900px로
  접힌다. `agent-browser`는 화면 밖 요소 클릭을 조용히 빗나가므로 결과가 끝내 안 떴다. `[5/8]` CTA와
  같은 부류라 같은 처방(`scrollIntoView` 후 클릭). 스크롤 후엔 정상 계산된다(헤드라인 348만 원 확인)
- **E2E `[9/9]` 프리필 단언이 단위를 틀렸다** — 프리필은 정상 동작했는데(`equity:5000`·`revenue:1237`)
  스크립트가 원 단위(`50000000`)를 기다려 15초 타임아웃으로 죽었다. 폼 금액 입력은 화면 표기와 같은
  **만원 단위**이고(`wonToManwon` + "만원" 라벨) 내부 상태·API만 원 단위다. 필수 빈칸을 채우는 native
  setter도 원 단위 값(`20000000` = 2,000억)을 넣고 있었다 — 만원 단위(2000·2000·2500, 대구 시연 사례와
  같은 값)로 고치고 주석에 경계를 적었다.
- `AB eval`이 결과를 JSON 인코딩하므로 `JSON.stringify` 객체는 이스케이프된 따옴표 때문에 부분 문자열
  매칭이 빗나간다. `[1/8]` 클릭 좌표처럼 공백 구분 숫자로 받아 `tr`로 씻는 방식으로 통일했다.
- 랜딩·analysis-api 병합 뒤 E2E가 드러낸 선택자·계약 회귀 3건째다(v0.18.2 화면 밖 클릭, v0.18.3 select,
  이번 단위). 전부 제품이 아니라 스크립트가 변화를 못 따라온 경우다
...
### Added
- **사이드패널 서사 재배열** (무대 설계서 §5) — 카드 나열이던 패널을 창업자의 질문 순서로 다시 세운다:
  ① 어떤 동네인가(동네 유형·근거) → ② 하루가 어떻게 흐르나(4블록 막대) → ③ 내 업종은 언제 돈이 도나
  (시간대 두 선) → ④ 얼마나 버티나 → ⑤ 업종 실적(기존 카드 + 스냅샷 안내) → 업종별 섹션 → AI 분석.
  관문(T1)이 동을 선택한 채 내려놓으므로 이 패널이 첫 화면이다. 동네 맥락이 먼저, 업종 상세가 뒤.
  섹션 `aria-label` 순서를 테스트로 고정
- **② 4블록 막대** `time-block-bars.tsx` — 배치가 준 `block_intensities`(v0.32.0)를 손 SVG `<rect>` 4개로
  그린다. 1.00 기준선(점선), 정점 블록만 강조색. **화면은 시간당 보정을 다시 하지 않는다** — 두 곳에서
  하면 언젠가 한 곳이 틀린다. DOM SVG는 `var()`를 읽으므로 계산된 색을 넘길 필요가 없다(MapLibre와 다름).
  정점→바닥 서사(`phasesNarrative` ?? `timeLabelSentence`)를 곁들인다
- **③ 시간대 두 선** `hour-gap-chart.tsx` + `lib/hour-gap-sentence.ts` — `GET /hour-gaps`(v0.32.0)의 6구간을
  유동·매출 두 `<polyline>`으로 겹쳐 그린다. **gap 막대 하나만 그리지 않는다** — v0.26.0 검증에서 어긋남의
  부호는 절대값이 아니라 상대 위치에 있음이 확인됐다(업무형 카페 06_11 gap은 음수지만 6유형 중 1위).
  문장도 결정론: 유동 최대·매출 최대 구간이 다르면 "사람은 {구간}에 가장 많고, 돈은 {구간}에 돕니다",
  같으면 "사람과 돈이 {구간}에 같이 몰립니다". 구간 표기는 백엔드 intent BC 어휘와 같다(관문 진단과
  어긋나지 않게). 매출 자료가 없는 조합(404 `HOUR_GAP_NOT_FOUND`)은 한 줄로 말한다
- **④ 얼마나 버티나** `staying-power.tsx` — `GET /commerce-changes/{region}`(v0.32.0). 영업 개월·폐업까지
  개월을 **같은 분기 서울 평균과 나란히** 놓는다("110개월 · 서울 118" — 서울이 옆에 있어야 110이 읽힌다).
  상권변화 배지는 원천 `change_name` 그대로, 색은 코드→토큰 테이블(LL accent·LH ok·HH secondary·HL danger).
  결측은 "집계 없음", `seoul`이 null이면 서울 절을 뺀다
- **sticky CTA** — 패널이 길어져 "AI 분석 →"이 스크롤 아래로 접히던 것(v0.18.2 E2E 재현)을 `aside` 하단에
  `position: sticky`로 고정. 패딩을 음수 마진으로 뚫고 그라데이션 배경으로 가린다
- `hooks/use-region-profile.ts` — ①·②가 같은 조회 키를 쓰므로 프로필 요청은 한 번이다
- `shared/neighborhood.ts`에 `HOUR_BAND_LABELS`·`TIME_BLOCK_LABELS`, `shared/korean.ts`(은/는 조사 —
  "카페은"이 나오던 제목을 고쳤다)
- mock `/api/mock/commerce-changes/[regionCode]`(서울 평균 동봉, 단계구분도와 같은 해시라 지도 색과 패널
  숫자가 일치 — 테스트로 고정) · `/api/mock/hour-gaps`(유형별 고정 모양, 강도의 구간 길이 가중 평균이 1인지
  테스트, 어린이집 404) · `regionProfileOf`에 `block_intensities`

### Validation
- 테스트 26건 추가 → vitest **191/191**(46파일), `tsc` clean. E2E는 `bash -n`만(실행은 T2 끝에)
- 실 dev 서버(3200→8201) `/map?region=1168064000&industry=cafe`: 섹션 순서 `동네 유형 → 하루 흐름 → 업종 시간대
  → 얼마나 버티나 → 업종 실적`, 정점 막대 `day`, 서사 "점심·오후에 정점을 찍고, 밤이면 텅 빕니다",
  어긋남 문장 "사람과 돈이 점심(11~14시)에 같이 몰립니다"(유동 11_14·14_17 동률 → 앞 구간), 버티기
  "110개월 · 서울 118 / 48개월 · 서울 54", CTA `position: sticky`이고 뷰포트 안에 보임
