#!/usr/bin/env bash
# 한국은행 기준금리 수집기 크론 러너 — 주 1회(월 05:20) 실행 (등록: crontab)
# ECOS 722Y001 월별 기준금리 1회 호출 전량 수신 → interest_rate 업서트(멱등)
set -euo pipefail
...
} >> "${LOG_FILE}" 2>&1 || echo "[$(date '+%Y-%m-%d %H:%M:%S')] interest rate collector 실패 (exit $?)" >> "${LOG_FILE}"
