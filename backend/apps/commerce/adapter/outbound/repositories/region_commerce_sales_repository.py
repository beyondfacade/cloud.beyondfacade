...
from apps.commerce.adapter.outbound.orm_mappers.region_commerce_sales_orm_mapper import to_row
...
_BATCH = 9000  # 6컬럼 × 9000 = 54,000 파라미터 < psycopg 한도 65,535 (tobacco 전례)


class SqlAlchemyRegionCommerceSalesRepository(RegionCommerceSalesRepositoryPort):
    def upsert(self, rows: list[RegionCommerceSales]) -> int:
        ...
        with session_scope() as session:
            for start in range(0, len(rows), _BATCH):
                ...
                statement = insert(RegionCommerceSalesOrm).values(values)
                session.execute(
