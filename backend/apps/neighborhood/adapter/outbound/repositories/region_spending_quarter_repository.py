...
from apps.neighborhood.adapter.outbound.orm_mappers.region_spending_quarter_orm_mapper import (
    to_row,
)
...
_PK = ("adstrd_code", "year_quarter", "spending_category")
_BATCH = 12000  # 5컬럼 × 12000 = 60,000 파라미터 < psycopg 한도 65,535


class SqlAlchemyRegionSpendingQuarterRepository(RegionSpendingQuarterRepositoryPort):
    def upsert(self, rows: list[RegionSpendingQuarter]) -> int:
        ...
        with session_scope() as session:
            for start in range(0, len(rows), _BATCH):
                ...
                statement = insert(RegionSpendingQuarterOrm).values(values)
                session.execute(
