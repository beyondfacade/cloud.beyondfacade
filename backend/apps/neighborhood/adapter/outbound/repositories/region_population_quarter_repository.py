...
from apps.neighborhood.adapter.outbound.orm_mappers.region_population_quarter_orm_mapper import (
    to_row,
)
...
_PK = ("adstrd_code", "year_quarter", "population_type", "dim_type", "dim_key")
_BATCH = 8000  # 7컬럼 × 8000 = 56,000 파라미터 < psycopg 한도 65,535


class SqlAlchemyRegionPopulationQuarterRepository(RegionPopulationQuarterRepositoryPort):
    def upsert(self, rows: list[RegionPopulationQuarter]) -> int:
        ...
        with session_scope() as session:
            for start in range(0, len(rows), _BATCH):
                ...
                statement = insert(RegionPopulationQuarterOrm).values(values)
                session.execute(
