...
from apps.neighborhood.adapter.outbound.orm_mappers.region_housing_average_quarter_orm_mapper import (
    to_row,
)
...
class SqlAlchemyRegionHousingAverageQuarterRepository(RegionHousingAverageQuarterRepositoryPort):
    def upsert(self, rows: list[RegionHousingAverageQuarter]) -> int:
        ...
        with session_scope() as session:
            for start in range(0, len(rows), _BATCH):
                ...
                statement = insert(RegionHousingAverageQuarterOrm).values(values)
                session.execute(
