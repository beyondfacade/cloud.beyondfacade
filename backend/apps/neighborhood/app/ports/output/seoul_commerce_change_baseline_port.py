"""Driven Ports — seoul_commerce_change_baseline 적재가 요구하는 계약 (ISP: 원천 읽기 / 저장 분리).
...
class SeoulCommerceChangeBaselineGatewayPort(ABC):
    @abstractmethod
    def fetch_baselines(self, path: Path) -> Iterator[SeoulCommerceChangeBaseline]:
        """원천 CSV 1개를 읽어 엔티티를 흘려보낸다."""


class SeoulCommerceChangeBaselineRepositoryPort(ABC):
    @abstractmethod
    def upsert(self, rows: list[SeoulCommerceChangeBaseline]) -> int:
        """PK(year_quarter) 기준 멱등 업서트. 처리 건수 반환."""
