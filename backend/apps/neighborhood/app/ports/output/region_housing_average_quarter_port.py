"""Driven Ports — region_housing_average_quarter 적재가 요구하는 계약 (ISP: 원천 읽기 / 저장 분리).
...
class RegionHousingAverageQuarterGatewayPort(ABC):
    @abstractmethod
...
class RegionHousingAverageQuarterRepositoryPort(ABC):
    @abstractmethod
    def upsert(self, rows: list[RegionHousingAverageQuarter]) -> int:
        """PK(adstrd_code, year_quarter) 기준 멱등 업서트. 처리 건수 반환."""
