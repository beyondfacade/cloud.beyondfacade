...
### Added
- **뉴스 검색 결과에서 같은 사건 기사 접기** — `apps/rag/domain/services/same_event_collapser.py`(순수 함수:
  제목 토큰 Jaccard ≥ 0.3 이고 보도일 차 ≤ 3일이면 같은 사건, 점수 높은 기사만 대표로 남김). `RagSearchInteractor.search`가
  source_type별 Strategy 테이블(`_COLLAPSERS = {"news": …}`)로 뉴스만 top_k×10을 가져와 접은 뒤 top_k를 돌려준다.
  공고는 차수·연도별 공고가 서로 다른 문서라 접지 않는다. 색인·스키마 변경 없음 — 증분 크론에서 대표가 흔들리지 않는다.
  에이전트 `search_news` 도구가 그대로 혜택을 본다
- 테스트 7건: 접기 순수 함수 5(같은 보도자료·다른 사건 순서 유지·보도일 창·보도일 없음·빈 입력) + 인터랙터 2

### 실측 (평가셋 뉴스 40문항, fp16)
- top-5 중 같은 사건 기사 수 **평균 2.98 → 2.00**, 2건 이상 점유 27 → 18문항, 5건 전부 같은 사건 12 → 4문항.
  Hit@5 40/40·1위 31/40 유지, 본지표 confirmed 180건 fp16 Hit@5 1.000·MRR 0.950 / ollama 1.000·0.946 (변화 없음)
- 더 공격적인 규칙은 전부 기각 — 제목 J 0.2(Hit 38/40), 앵커 토큰 2개 공유(잘못 합침 231→587), 히트 간 임베딩
  코사인(클러스터 내부 중앙값 0.848 vs 외부 p90 0.916으로 분리 불가), 요약문 J OR(Hit 38). 남은 5/5 4문항(신중앙시장
  착공·제주 성수·올리브영 남포·부산 추경)은 제목 표현이 제각각인 대형 보도자료라 어휘 규칙으론 못 잡는다 —
  다음 단계는 수집 시 사건 클러스터링(LLM 또는 본문 기반)

## [v0.37.1] - 2026-09-24

### Changed
- **평가 지표 Recall@5 → Hit@5** (`evaluate_rag.hit_at_k`) — relevant_ids를 "이 중 하나라도 맞으면 정답"인 동치 집합으로
  정의한다. 뉴스는 같은 보도자료를 받은 기사가 3~50건이라(실측: 올리브영 남포 51건·신중앙시장 착공 34건·오세훈 추석
  15건) 단일 정답으로 두면 검색기가 같은 사건의 다른 기사를 1위로 올려도 틀린 것이 됐다. 단일 정답 행에선 Recall@5 ==
  Hit@5라 이전 수치와 비교 가능. 결과 JSON 키 `recall_at_5` → `hit_at_5`. 테스트 5건으로 재작성(다중 정답·top-k 밖)

### 평가셋 (data, 9/24 밤)
- 뉴스 40건 중 29건에 같은 사건 기사를 relevant_ids로 묶음(세션이 제목·날짜로 확정, 2~51건). 보류 X 8건 → O
  (168번은 "같은 날짜 기사 1건 더"가 작성 메모 오류였음 — 다른 날짜 소식). 뉴스 MRR **0.578 → 0.865**
- Fable 권고대로 수치·고유명사 과다 질문 10건(55·85·87·88·95·114·126·130·136·149) + 155(인천 추가)를 지원 유형·
  대상 조건 수준으로 완화 → **재판정 대상(candidate 11)**. 완화 후에도 fp16 top-5 11/11, 1위 10/11
- 본지표(confirmed 169): **fp16 Hit@5 1.000·MRR 0.945 / ollama 1.000·0.941**. 원천별 fp16 — funding 129건 MRR 0.971,
  news 40건 MRR 0.865
...
### Changed
- **평가셋 판정 기준을 하나로 통일** — 사람 검수(9/24)가 Claude 1차 판정에서 불일치 10건을 찾았다: 통합공고 3건
  (4·8·13)은 같은 성격의 12·17을 X로 두면서 O였고, 지원방식 누락 4건(3·37·40·47), 조건 누락 1건(23),
  이웃 행과 불일치 2건(43·46). 기준: "질문에 담긴 정보만으로 이 공고가 다른 유사 공고보다 우선적으로 정답이 될 수
  있어야 O, 여러 공고가 자연스럽게 정답이면 X. 통합공고도 예외 없음('한눈에'·'통합 안내' 의도가 드러날 때만 O).
  지원방식이 공고 핵심이면 질문에 담겨야 한다." `judge_evalset` 시스템 프롬프트에 이 기준을 그대로 넣었다
- 시트(`data/eval/rag_evalset_review.md`) 상단에 기준을 적고 10건을 `판정: X  # 사람 검수: 사유 | 제안: 질문`으로 기입

### 실측
- 최종 **confirmed 30 / rejected 20**. 본지표 — **fp16 Recall@5 1.000 · MRR 0.923 / ollama Recall@5 1.000 · MRR 0.900**
  (1차 판정 40건 기준 0.950/0.868 → 일반적 질문이 빠지며 상승). X 20건은 시트의 제안 질문을 채택하면 O로 전환 가능
...
### Fixed
- **SGIS 지오코딩에서 주소 하나가 잘못되면 배치 전체가 죽던 것** — `errCd -200`("검색할 주소를
  확인해주세요", 주소 형식 불량)을 토큰 만료와 같은 재시도 대상으로 분류한 뒤 `RuntimeError`를
  던졌는데, `GeocodeStoresInteractor`는 `geocode()`를 `try`로 감싸지 않는다. 한 건의 불량 주소가
  남은 전량을 중단시킨다. **실호출로 재현**(2026-09-24, 키 검증 중 세 번째 주소에서 발생)
- `-200`을 `-100`(검색결과 없음)과 같이 **건너뛰기**로 바꿨다. 인터랙터가 지번 주소로 한 번 더
  시도하는 경로는 그대로 살아 있다. 재시도 목록에는 인증·토큰 오류(`-401`·`-1001`)만 남긴다 —
  `-200`은 재시도해도 같은 답이라 호출만 두 번 쓴다
- 회귀 테스트 1건(호출 수가 1인지까지 단언 — 재시도 낭비 방지)

### Validation
- 실호출: 테헤란로 152 → (37.5000, 127.0365) · 양화로 45 → (37.5515, 126.9140) · 불량 주소 → None.
  총 3회 호출로 배치가 끊기지 않는다
- 전체 **505 passed**

### 확인 사항 — SGIS 파이프라인은 이미 완료 상태
사용자 요청으로 키를 점검하며 함께 확인했다. `SGIS_SERVICE_ID`/`SGIS_SECURITY_KEY` 정상 동작하고,
지오코딩·region 배정·지표 집계는 `feature/analysis-api`(`e573e3f`→`4f6f1ec`→`be31057`)에서 이미
완료돼 T0-2 병합으로 따라왔다. 실측 academy 25,554 중 좌표 25,504(99.8%)·region 25,504,
real_estate 25,435 중 좌표 25,299(99.5%). `region_industry_metric`에 academy 3,408행·
real_estate 3,416행(10업종 전부 2026년까지). 남은 미좌표는 **주소가 아예 없는 행**(학원 50 중 46,
부동산 136 중 134)과 폐업 업소(부동산 133)다. `docs/HANDOFF.md` 9/17판의 "차단 2건"은 낡은 기록이다.

## [v0.35.0] - 2026-09-24

### Added
- **리포트 두뇌 혼합 배선** — 기본을 Gemini로 하되 실패하면 로컬로 내려간다. 사용자 결정 사항이었고
  (`v0.21.1` "두뇌 채택은 사용자 결정") 근거는 두뇌 비교 실측이다
  (`data/eval/results/agent_compare.md`, 시나리오 10)

  | 지표 | gemma4:12b(로컬) | gemini-2.5-flash |
  |---|---:|---:|
  | 5섹션 완성률 | **50%** | **90%** |
  | 평균 소요 | 39.9s | 17.4s |
  | 평균 토큰 | 10,997 | 7,570 |

  그 측정은 도구 7종·계약 블록 0개 시절이다. 지금은 **도구 9종 + 계약 블록 2개**(market·calculator)라
  로컬 쪽 입력 토큰이 29,547까지 불었고, v0.33.1 조사에서 5섹션이 전부 폴백 문구로 나오는 실행을
  관측했다. 로컬을 버리지 않는 이유는 **키가 없는 환경**(도커 8200 조회 전용)과 쿼터 소진·일시 장애다
  - `adapter/outbound/llm/fallback_llm_adapter.py` — `LLMGatewayPort`를 구현하고 primary·secondary를
    감싸는 한 겹(GoF Decorator). if/elif 분기가 아니다
  - **폴백하는 경우**: primary 어댑터 생성 실패(키 없음)·호출 예외(인증 오류·429 예산 소진·타임아웃).
    **폴백하지 않는 경우**: primary가 정상 응답했는데 내용이 빈약하거나 섹션이 모자란 것 — 그건 장애가
    아니라 품질이고 어댑터가 판단할 일이 아니다
  - **키 유무는 `GeminiLLMAdapter` 생성이 던지는 `ValueError`로 판정한다.** 키 값을 읽지도 로그에
    남기지도 않는다. 생성 실패는 한 번만 시도하고(턴마다 재시도하지 않는다), 호출 실패는 매 턴 다시
    시도한다 — 쿼터·일시 장애는 회복될 수 있다
  - **`model_name`은 실제로 답한 쪽을 가리킨다.** 라우터가 run 이후 `_llm.model_name`을 읽어
    `analysis_report.model`에 남기므로, 폴백이 일어나면 그 사실이 기록에 남는다. 새 SSE 이벤트 타입을
    만들지 않았다 — 프론트 `AgentName` 유니온을 깨뜨리지 않기 위해서다
  - Composition Root에 `hybrid` 추가·**기본값 전환**. `gemma3`·`gemini` 직접 지정은 남긴다
    (두뇌 비교 평가 러너 `run_agent_eval --model`이 쓴다). 요청 스키마 `model` 패턴·기본값도 갱신

### Fixed
- **`GET /funding` 목록이 마감 지난 공고를 보여줄 수 있던 것** — `is_expired` 플래그만 보는데 이 값은
  일 배치(`refresh_expirations`, 매일 05:10)가 갱신하므로 설계상 최대 하루 낡는다. 어제 마감한 공고가
  다음 배치까지 "모집 중"으로 남았다. 후보 필터(v0.34.0 `domain/services/candidates.py`)가 이미
  쓰는 방어(`is_past_deadline`)를 목록에도 둔다

### 크론 점검 (T4-1 관찰 후속)
만료 갱신 배치는 **정상이다.** `crontab`에 등록돼 있고(`10 5 * * *`) 오늘 05:10 실행에서 510건을
만료 처리했다. 현재 DB에 "미만료인데 마감 지난" 행은 **0건**이다. T4-1이 본 것은 배치 직전 창의
상태였다 — 결함이 아니라 일 배치의 24시간 해상도다. 위 Fixed가 그 창을 메운다

### Validation
- 테스트 11건 추가(폴백 7 · 배선 3 · 목록 마감 방어 1) → **499 passed**
- 실호출(8201, 실 Gemini): **20초 · 5섹션 전부 실제 내용**(294·620·421·426·153자) · 도구 6회 · 인용 11건.
  `analysis_report.model` = `gemini-2.5-flash`, 직전(변경 전) 실행은 `gemma4:12b`에 본문 136자였다
- `calculator` 절은 입력이 없으니 계산하지 않고 `/plan` 안내로 끝냈다 — v0.33.0 (2/2) 계약대로다
- 기존 실패 1건(`test_broker_ingest::test_first_load_never_estimates_closure`)은 전체 실행에서만 나고
  격리 실행은 통과한다. 내 변경을 스태시해도 동일해 **기존 테스트 오염**이다(이번 작업과 무관)

### (2/2) 리포트 `funding` 절 — 결정론 후보 (T4-2)
- **도구 `get_funding_candidates`** (9종 → 10종) — v0.34.0의 결정론 필터를 `FundingFactsPort`(ISP,
  `FinanceFactsPort` 전례) 뒤에서 호출한다. RAG `search_funding`(유사도)은 남긴다 — 역할이 다르다.
  유사도는 "비슷한 글"을 찾고 이쪽은 지역·대상·마감을 확정적으로 판정한다
  - `stage`는 기존 어휘 `funding`을 쓴다. SSE `agent_status`의 값은 프론트 `AgentName` 합집합에
    묶여 있어 새 stage를 내면 진행 패널이 모르는 값을 받는다(v0.33.0에서 확인한 제약)
  - **필수 인자가 없다.** 라우터도 `industry`·`need`·`stage`를 전부 선택으로 받는다 — 인자 없이
    불러도 서울/전국 후보가 나온다. 억지 required를 세우면 스키마가 거짓말이 되고 LLM이 값을
    지어낸다. 기존 "모든 도구는 required를 갖는다" 테스트에 이 예외를 명시했다
  - 인용은 `fact` 등급 + 원문 링크 — RAG 신호가 아니라 규칙 판정 결과다
  - 게이트웨이가 `disclaimer`("자격 확정이 아니라 해당 가능성")를 결과에 함께 싣는다. `caveats`와
    같은 원칙 — 해석 금지 사항은 수치 옆에 둔다
- **`[funding 섹션 출력 계약]`** — 후보는 자격 확정이 아니라고 쓰고 원문 링크를 남긴다. 금리·한도는
  규칙 ③대로 예상치 고지. `external_funding_need`를 알면 그 금액이 이 절의 헤드라인이다
- 실호출: `cafe`·조달 필요 3,160만·`pre` → 후보 8건 전부 서울특별시 주관, `why`에 걸린 규칙 표기
- 테스트 4건 추가 → 전체 **504 passed**

## [v0.34.0] - 2026-09-24

### Added
- **후보 공고 필터** `GET /funding/candidates?industry=&need=&stage=` (설계서 §3) — 서울 창업자에게
  해당하는 미만료 공고 상위 8건. 결정론 필터, LLM 없음. 상품 목록이 아니라 "상담에서 물어볼 만한 것"의
  후보이며 자격 확정이 아니다
  - `domain/services/region_filter.py` — 시도 커버리지로 서울 전용·전국·제외를 가른다.
    **원천이 시도 어휘를 두 벌 섞어 쓴다**: `전남광주`(통합 명칭) 512건과 `광주`·`전남`(282·281건)이
    공존해 전국 공고의 지역 태그가 16개거나 17개다. 태그 수만 세면 16개짜리 353건이 전국으로 안 잡혀
    "서울 전용"으로 오분류된다 — `전남광주`를 펴서 17개 기준으로 센다. `전국` 태그는 3건뿐이라 그것만
    믿을 수 없다. 설계서의 73/45는 이 오분류 상태의 수치라 70/490으로 정정했다
  - **소관기관이 서울 밖 지자체면 태그와 무관하게 제외** — 과천시 이자차액보전처럼 시도를 전부 달아둔
    지자체 공고가 26건 걸린다. 서울 자치구 25개는 `district` 마스터로 판정한다(하드코딩하면 마스터와
    어긋날 자리가 생긴다)
  - **만료는 `is_expired` 플래그와 `deadline`을 둘 다 본다.** 플래그는 일 배치가 갱신하므로 배치가
    늦으면 마감 지난 공고가 남는다 — 실제로 오늘(9/24) 기준 9월 초 마감 공고가 섞여 나왔다.
    상담 자료에 지난 공고를 넣는 것은 그 자체로 결함이다
  - 정렬: 서울 전용 → 전국, 우선 분야(금융·창업·경영) → 그 외, 창업 단계 가점, 마감 임박 순(상시는 뒤).
    각 항목에 `why`("서울 · 소상공인 · 금융"). 대상·분야가 같은 말이면 한 번만 쓴다
  - **`industry`·`need`는 필터에 쓰지 않고 응답에 되돌리기만 한다** — 공고에 업종·한도가 구조화돼
    있지 않다. 있는 척하지 않는다(구조화는 T4-3)
- **확인할 질문** `POST /finance/questions` (설계서 §4) — 계획 수치에서 결정론으로 만드는 초안 11규칙.
  `domain/services/questions.py`의 **규칙 객체 리스트**(Chain of Responsibility), if/elif 없음
  - gap 2(조달 필요·부족액) · assumption 5(대출 조건·비관 런웨이·미확인 금액·프리필 월매출·프리필 월세)
    · procedure 4+(사업자등록·보증·정책자금 확인서·후보 공고 상위 3)
  - 모든 질문이 `basis`를 갖는다 — 어느 수치에서 나왔는지 말할 수 있어야 한다
  - 금액은 화면과 같은 **만원 단위**, 조사는 받침 규칙(intent BC와 같은 사본)
  - **요청에 계산 결과를 받지 않는다.** 서버가 `input`으로 다시 계산한다 — `POST /finance/simulate`와
    같은 원칙이다(클라이언트 계산을 신뢰하지 않는다). 설계서 §4의 요청 형태에서 `result`를 뺐다
  - **후보 공고 제목은 요청으로 받는다** — finance BC가 funding BC를 직접 읽지 않는다. 화면이
    `GET /funding/candidates` 결과를 실어 보낸다. 두 BC를 묶으면 계획 계산이 공고 적재에 묶인다

### Validation
- 테스트 51건 추가(후보 22 · 질문 29 + 라우터 5) → 전체 **487 passed**.
  `domain/`·`app/`에 프레임워크 import 없음
- 실호출: `/funding/candidates?stage=pre` 8건 전부 `서울특별시` 주관 서울 전용 공고, 마감일 전부
  오늘 이후 또는 상시. `stage=registered`는 소상공인 공고가 앞선다.
  `/finance/questions` 시연 사례 + 후보 3건 → 질문 12개(gap 2 · assumption 4 · procedure 6)

## [v0.33.1] - 2026-09-24

### Fixed
- **에이전트 루프가 9분씩 돌다 빈 리포트를 내는 일** — `_MAX_TURNS=12`는 턴만 세고 시간을 세지
  않는다. 로컬 `gemma4:12b`는 한 턴이 30~60초라(입력 ~21k 토큰) 모델이 도구를 맴돌며 12턴을 다
  쓰면 9분이 넘고, 그 사이 클라이언트가 먼저 끊는다. 실측 대조:

  | 실행 | 소요 | 리포트 |
  |---|---|---|
  | 정상 | 41.8s · 45.9s · 98.5s | 997자 · 1,302자 · 275자 |
  | 실패 | **534.7s · 558.4s** | **0자** |

  Ollama에 모델을 미리 상주시켜도 재현됐다 — 콜드 로드가 아니라 루프가 원인이다. 도구 수집에
  **벽시계 예산 180초**를 둬 마무리 턴을 반드시 남긴다. 예산을 넘기면 수집을 멈추고
  `_FINAL_REQUEST`로 "지금까지 모은 것만으로" 리포트를 쓰게 한다(리포트가 비는 대신 짧아진다).
  시계는 생성자 주입이라 테스트가 기다리지 않는다. 턴 한도(12)는 그대로 — 둘은 다른 것을 막는다
- 근본 원인은 아니지만 함께 관찰된 것: 모델이 같은 도구를 되풀이 호출하고(`get_neighborhood_profile`
  ×2 · `search_shocks` ×2) 업종 인자에 서비스업종코드(`211101`)를 넣는 등 헤맨다. 도구가 7종 → 9종으로,
  시스템 프롬프트가 계약 두 블록으로 늘어난 뒤의 현상이다. 프롬프트·도구 다이어트는 별도 과제

- **LLM 호출이 터지면 리포트가 아예 안 뜨던 것** — `_chat`에 예외 처리가 없어 어댑터의 120초
  타임아웃(`httpx`)이나 LLM 장애가 제너레이터를 죽였다. `report_delta`가 하나도 안 나가 화면은
  `NO_ARTICLE` 상태로 남고, 사용자는 "끝났는지 도는 중인지"조차 모른다. 수집 턴이 터지면 마무리로
  넘어가고, 마무리까지 터지면 폴백 섹션("분석 데이터가 부족합니다")으로 낸다 — **스트림은 항상
  리포트로 끝난다.** 빈 화면보다 빈약한 리포트가 낫다
- 벽시계 예산은 턴 **사이**만 막는다. 한 번의 호출이 길어지는 것은 어댑터 타임아웃(120초)이 막고,
  그 타임아웃이 스트림을 죽이던 것을 위 예외 처리가 막는다 — 셋이 각각 다른 구멍이다

### Validation
- 테스트 4건 추가(예산 초과 시 수집 중단 + 마무리 턴 도달 / 예산 안에서는 기존 동작 /
  수집 턴 예외 시에도 5섹션 방출 / 마무리 턴까지 실패하면 폴백), 전체 **433 passed**
